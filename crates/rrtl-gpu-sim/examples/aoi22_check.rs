//! Measures the four-operand standard cells AOI22 = ~((a&b)|(c&d)) and
//! OAI22 = ~((a|b)&(c|d)) on a cell-tree design. Each cell is FOUR ops today
//! (two binops + a binop + a not); the fused cell is one op (3rd/4th operand in aux).
//! Reports GPU throughput fusion on vs off + correctness.
//!
//! Usage: cargo run --release -p rrtl-gpu-sim --example aoi22_check -- [bits width depth lanes steps]
...
fn build(bits: u32, width: usize, depth: usize) -> Design {
    ...
    for lane in 0..width {
        ...
        for k in 0..depth {
            x = if k % 2 == 0 {
                !((x & a.value()) | (b.value() & c.value()))   // AOI22
            } else {
                !((x | a.value()) & (b.value() | c.value()))   // OAI22
            };
        }
        m.next(acc, x);
...
fn main() {
    ...
    let (a, bb, cc, clk, o0) = (sig("a"), sig("b"), sig("c"), sig("clk"), sig("o0"));
    ...
    let run = |prog: &InterpProgram| -> (f64, Vec<u32>) {
        ...
        for (s, mlt) in [(a, 2654435761u32), (bb, 40503), (cc, 2246822519)] {
            gpu.set_signal(off(s), &(0..lanes as u32).map(|l| l.wrapping_mul(mlt)+1).collect::<Vec<_>>());
        }
