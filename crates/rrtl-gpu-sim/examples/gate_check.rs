//! Measures the gate-level fused ops (NAND/NOR/XNOR) on a NAND/NOR/XNOR-tree
//! datapath standing in for a gate-level netlist, where every `~(a op b)` is
//! today two ops (a binop + a not). Fusing them ~halves the op count. Reports
//! GPU throughput fusion on vs off and bit-exactness.
//!
//! Usage: cargo run --release -p rrtl-gpu-sim --example gate_check -- [bits width depth lanes steps]
...
fn build(bits: u32, width: usize, depth: usize) -> Design {
    ...
    for lane in 0..width {
        ...
        let mut x = acc.value();
        for k in 0..depth {
            // alternate NAND / NOR / XNOR -> Not(And/Or/Xor)
            x = match k % 3 {
                0 => !(x & a.value()),
                1 => !(x | a.value()),
                _ => !(x ^ a.value()),
...
fn main() {
    ...
    let (width, depth, lanes, steps) = (p(2, 64), p(3, 12), p(4, 16384), p(5, 64));
    ...
    let (pt, po) = run(&plain);
    let (ft, fo) = run(&fused);
    println!("  GPU plain {:>6.2} Mlc/s   fused {:>6.2} Mlc/s   ({:.2}x)   [{}]",
        mlc(pt), mlc(ft), pt / ft, if po == fo { "OK" } else { "MISMATCH" });
}
